...
from src.gui.widgets.tree_widget import TreeWidget
from src.utils.classes import NaNValidator
import json, gc
import numpy as np
import re
import copy
...
class LeftDockWindow(QWidget):
    ...
    def create_left_dock_window(self, parent: Optional[QWidget]) -> None:
        ...
        LeftDockWindow.tree_widget.expandAll()
    ...
    def create_details_tree_widget(self) -> None:
        ...
        self.details_tree.setWordWrap(True)
        ...
        self.details_layout.setContentsMargins(0, 0, 0, 7)
        self.details_layout.setSpacing(0)
        self.details_tree.resizeEvent = self.on_details_tree_resize
        
    def on_details_tree_resize(self, event):
        """
        Handle resize events to adjust column widths dynamically.
        """
        super(QTreeWidget, self.details_tree).resizeEvent(event)
        
        if hasattr(self, 'details_tree') and self.details_tree.columnCount() > 0:
            total_width = self.details_tree.width()
            if total_width > 0:
                self.details_tree.setColumnWidth(0, int(total_width * 0.4))
                self.details_tree.setColumnWidth(1, int(total_width * 0.4))
                self.details_tree.setColumnWidth(2, int(total_width * 0.2))

...
class ResultsViewer:
    ...
    def generate_html_with_plotly(fig: Optional[go.Figure] = None, plot_id: str = "plotly-3Dplot", server_url: str = "http://localhost:8000") -> str:
        ...
        """
        plotly_script: str = f'<script src="{server_url}/js/plotly-3.0.1.min.js"></script>'
    ...
    def display_stiffness_matrix_with_solution(F: Dict[int, Dict[str, float]], 
        ...
        for i in range(rows):
            ...
            for j in range(cols):
                ...
                if theme == "light":
                    diagonal_color = QColor(139, 89, 89) 
                    zero_color = QColor(220, 220, 220)    
                else:  
                    diagonal_color = QColor(255, 128, 0) 
                    zero_color = QColor(40, 40, 40)      
                ...
                if theme == "light":
                    ...
                    base_color = 30
                    max_color_contribution = 100
                    
                    red: int = base_color + int(max_color_contribution * intensity * 0.5) 
                    green: int = base_color + int(max_color_contribution * intensity * 0.9) 
                    blue: int = base_color + int(max_color_contribution * intensity) 
                ...
                if i == j:
                    item.setBackground(diagonal_color)
                elif value == 0:
                    item.setBackground(zero_color)
